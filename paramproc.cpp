...
{
	//errproc(pt==NOPARAM," This constructor can only work with Param:NOPARAM\n");
			...
			if (strcmp(start,_key)==0)
			{
				...
				if (n+1 < argc)
				{
					...
					switch (_pt)
					{
						...
						case UNSIGNED : _u=strtoul(argv[n+1],(char**)NULL,10); break;
						case DOUBLE   : _d=atof(argv[n+1]); break;
						case STRING   : _s=argv[n+1];       break;
						default : printf(" Unknown command line parameter");	//abkfatal(0," Unknown command line parameter");
					}
				}
			}
		}
...
{ 
	//errproc(found()," Parameter not found: you need to check for this first\n");
	return _on;
}
